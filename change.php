...
WHERE p.id ='.$_POST['id'].' ORDER BY `order`';
...
$requete1 = 'SELECT * FROM parallax where page_id ='.$_POST['id'].' AND `order` = 1';
$resultat = $pdo->prepare($requete1);
$resultat->execute();
$resultatParallaxTop = current($resultat->fetchAll(PDO::FETCH_ASSOC));

$requete1 = 'SELECT * FROM parallax where page_id ='.$_POST['id'].' AND `order` = 2';
$resultat = $pdo->prepare($requete1);
$resultat->execute();
$resultatParallaxMid = current($resultat->fetchAll(PDO::FETCH_ASSOC));

$requete1 = 'SELECT * FROM parallax where page_id ='.$_POST['id'].' AND `order` = 3';
$resultat = $pdo->prepare($requete1);
$resultat->execute();
